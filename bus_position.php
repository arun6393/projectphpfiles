        ...
        if($r==true) {
            ...
            if($result) {
                ...
                for($i=0; @$row=mysql_fetch_assoc($result); $i++) {
                    $flag=1;
                    //below line new
                    $rows[] = 	array(array("bus_no"=>$row['bus_no'],"lat"=>$row['bus_lat']),
				array("dest"=>$row['bus_long']));		
                		}
			
			array_multisort($rows[0],SORT_ASC,SORT_NUMERIC,
					$rows[1],SORT_NUMERIC,SORT_ASC);

                for($i1=0; @$row1=mysql_fetch_assoc($result1); $i1++) {
                    //$flag=1;
                    //below line new
                    $rows1[] = array("bus_stop"=>$row1['bus_stop'],"lat"=>$row1['latitude'],"long"=>$row1['longitude']);
                }
                if($flag=="0") {
